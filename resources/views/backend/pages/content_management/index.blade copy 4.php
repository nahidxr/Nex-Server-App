       ...
       $(document).ready(function () {
            ...
            $('#closeButton, #topCloseButton').on('click', function () {
                if (uploadedVideoPath) {
                    // Extract the relative file path (strip the URL part)
                    let relativeVideoPath = uploadedVideoPath.replace(window.location.origin + '/', '');

                    // Make an AJAX call to delete the video
                    $.ajax({
                        url: '{{ route("upload.delete") }}', // The route for deleting the video
                        method: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}',
                            video_path: relativeVideoPath // Send the relative file path
                        },
                ...
                resetUploadContainer(); // Reset the UI
                $('#uploadForm')[0].reset(); // Reset form inputs
            });
        });
