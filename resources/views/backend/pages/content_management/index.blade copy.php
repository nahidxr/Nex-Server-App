                ...
                <div class="modal-body">
                    <form id="uploadForm" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" id="videoFileInput" name="video" class="filepond" accept="video/*" required>
                        </div>
                        <!-- Hidden input to store folder name -->
                        <input type="hidden" id="folderInput" name="video">
                    </form>
    ...
    <script>
       $(document).ready(function() {
               ...
               server: {
                   process: {
                       ...
                       onload: function(response) {
                           // Set the folder name returned by the tempUpload function to the hidden input
                           $('#folderInput').val(response);
                       },
                   ...
                   revert: {
                        ...
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}' // Include CSRF token for DELETE request
                        },
                    },
               },
           });
           ...
           $('#saveButton').click(function() {
               ...
               $.ajax({
                   url: '/admin/store-content',
                   type: 'POST',
                   data: formData,
                   contentType: false,
                   processData: false,
                   success: function(response) {
                       $('#statusMessage').html('<div class="alert alert-success">' + response.success + '</div>');
            ...
            $('#uploadModal').on('hidden.bs.modal', function () {
               ...
               $('#statusMessage').empty(); // Clear any status messages
               $('#folderInput').val(''); // Clear the hidden folder input
               
               // Reset the FilePond instance if using FilePond
               pond.removeFiles();
           });
       });
